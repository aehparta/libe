...
#define s_box_inverse(index)    pgm_read_byte(&(_s_box_inverse[index]))
#define rcon(index)             pgm_read_byte(&(_rcon[index]))
#else
#define s_box(index)            _s_box[index]
#define s_box_inverse(index)    _s_box_inverse[index]
#define rcon(index)             _rcon[index]
#endif

// AES Tables

const uint8_t _s_box[256] TARGET_FLASH_MEMORY = {
...
const uint8_t _s_box_inverse[256] TARGET_FLASH_MEMORY = {
...
const uint8_t _rcon[10] TARGET_FLASH_MEMORY = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 };
...
static void aes_init(uint32_t *round_key, counter b, uint32_t *key, counter n)
{
  uint32_t t;
...
static void aes_encrypt(uint8_t *round_key, uint8_t block[16], counter rounds)
{
  counter i;
...
static void aes_decrypt(uint8_t *round_key, uint8_t block[16], counter rounds)
{
  counter i;
