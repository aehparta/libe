...
{
	uint8_t *p = &nvm_data[(intptr_t)(address)];

	// if (address == (void *)NVM_A_TYPE) {
	// 	/* special case: default_value to electrical */
	// 	return PEER_TYPE_HEATER_ELECTRICAL;
	// } else if (address >= (void *)NVM_A_VERIFICATION_CODE && address <= ((void *)NVM_A_VERIFICATION_CODE + 7)) {
	// 	/* special case: default_value to 12345678 */
	// 	return '1' + ((long int)address - (long int)NVM_A_VERIFICATION_CODE);
	// }
...
{
	uint16_t *p = (uint16_t *)&nvm_data[(intptr_t)address];
...
{
	uint32_t *p = (uint32_t *)&nvm_data[(intptr_t)address];
...
{
	float *p = (float *)&nvm_data[(intptr_t)address];
